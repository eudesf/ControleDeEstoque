...
public class RepositorioFuncionariosBDR implements RepositorioFuncionarios {
	...
	public void inserir(Funcionario funcionario) throws RepositorioException {
		try {
			...
			statement.executeUpdate(String.format(
					"INSERT INTO root_funcionarios VALUES "
							+ "('%s', %s, '%s', '%s', '%s', %s, %s, '%s')",
					funcionario.getNome(), trataData(funcionario
							.getDataCadastro()), funcionario.getEndereco(),
					funcionario.getMatricula(), funcionario.getCPF(),
					trataData(funcionario.getDataAdmissao()),
					trataData(funcionario.getDataDemissao()), funcionario
							.isAtivo() ? "1" : "0"));
	...
	public void remover(String matricula)
		...
		try {
			Statement statement = (Statement) pm.getCommunicationChannel();
			int i = statement
					.executeUpdate("DELETE FROM root_funcionarios WHERE matricula='"
							+ matricula + "'");
			if (i == 0) {
	...
	private Funcionario montaFuncionario(ResultSet resultSet)
			throws SQLException {
		Funcionario funcionario = new Funcionario(resultSet
				.getString("matricula"));
		...
		funcionario.setAtivo(resultSet.getString("ativo") != null
				&& resultSet.getString("ativo").equals("1"));
		return funcionario;
	}
	...
	public Funcionario procurar(String matricula)
		...
		try {
			...
			ResultSet resultSet = statement
					.executeQuery("SELECT * FROM root_funcionarios WHERE matricula = '"
							+ matricula + "'");
			if (resultSet.next()) {
		...
		try {
			...
			int i = statement
					.executeUpdate(String.format(
							"UPDATE root_funcionarios SET " + "nome = '%s', "
									+ "datacadastro = %s, "
									+ "endereco = '%s', "
									+ "matricula = '%s', " + "cpf = '%s', "
									+ "dataAdmissao = %s, "
							...
							funcionario.getNome(), trataData(funcionario
									.getDataCadastro()), funcionario
									.getEndereco(), funcionario.getMatricula(),
							funcionario.getCPF(), trataData(funcionario
									.getDataAdmissao()), trataData(funcionario
									.getDataDemissao()),
							funcionario.isAtivo() ? "1" : "0", funcionario
									.getMatricula()));
			if (i == 0) {
				throw new FuncionarioNaoEncontradoException(funcionario
						.getMatricula());
			}
		} catch (PersistenceMechanismException e) {
			throw new RepositorioException(e);
	...
	public RepositorioFuncionarios getFuncionarios()
		...
		try {
			Statement statement = (Statement) pm.getCommunicationChannel();
			ResultSet resultSet = statement
					.executeQuery("SELECT * FROM root_funcionarios");
